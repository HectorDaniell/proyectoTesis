...
def label_performance_throw(csv_file):
    """
    ...
    if max_displacement > 0:
        df['amplitude_score'] = df['wrist_displacement'] / max_displacement
    else:
        df['amplitude_score'] = 0.0
    
    # 3. Postural stability during throwing motion
    # Calculate hip movement to assess stability
    hip_movement = ((df['right_hip_x'].diff()**2 + df['right_hip_y'].diff()**2) ** 0.5).rolling(window=5).mean().fillna(0)
    hip_movement_max = hip_movement.max()
    # Lower hip movement indicates better stability
    # Handle division by zero and NaN values
    if hip_movement_max > 0:
        df['stability_score'] = (1 - hip_movement / hip_movement_max).fillna(0)
    else:
    ...
    print(f"Throw performance labels saved in {labeled_csv}")
